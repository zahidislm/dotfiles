...
// Helper 1: Calculate Luminance (brightness)
float lum(vec4 c) {
    return 0.299 * c.r + 0.587 * c.g + 0.114 * c.b;
}

// Helper 2: Calculate Saturation (color intensity vs grayscale)
float saturation(vec3 c) {
    float maxC = max(max(c.r, c.g), c.b);
    float minC = min(min(c.r, c.g), c.b);
    return (maxC <= 0.0) ? 0.0 : (maxC - minC) / maxC;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Snapshot the original alpha to prevent opacity bugs
    ...
    for (int i = 0; i < 24; i++) {
        ...
        float l = lum(c);
        float sat = saturation(c.rgb);

        // TWEAK THIS: 
        // sat > 0.3 means "ignore white/gray backgrounds"
        // l > 0.2 means "ignore dark colors"
        if (l > 0.2 && sat > 0.3) {
            color.rgb += l * s.z * c.rgb * 0.028;
        }
    }

    // Restore the Ghostty background-opacity
